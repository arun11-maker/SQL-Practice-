...
select dept , count(name) as empcount from employees group by dept order by empcount desc;
...
select dept , count(name) as empcount from employees group by dept having empcount>7;
...
select role , count(role) as rolecount from employees group by role having rolecount>5;
...
select dept , city ,count(name) as empcount from employees group by dept , city;
...
select dept , role , count(name) as totalemp from employees group by dept ,role having totalemp>1 order by dept;
...
select city , dept , count(name) as countemp from employees group by city , dept having countemp>1 order by countemp desc , city;
select role , sum(salary) as totalsalary from employees group by role having totalsalary>300000 order by totalsalary ;
select upper(name) as empname , substring(joindate ,1,4) as joinyear from employees having joinyear<=2019 order by joinyear;
select substring(joindate ,1,4) as joinyear , count(name) as joiners from employees group by joinyear order by joinyear;