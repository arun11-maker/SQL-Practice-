...
-- using exists
select * from customers as c where exists (select 1 from orders as o where o.customer_id=c.customer_id);

-- using not exists
select * from customers as c where not exists (select 1 from orders as o where o.customer_id=c.customer_id);

-- who have atleast 1 deliver order
...
(select  1 from orders as op where op.customer_id=c.customer_id and op.status='pending'));